...
public class CreateProjectAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		new CreateDialog("Create Project", shell) {
			@Override
			protected void okPressed() {
				try {
					...
					new Hudson(hudsonUrl.getText(), userName.getText(),
							password.getText()).create(projectName.getText(), selectedFile);
				} catch (IOException e) {
					Activator.getDefault().getLog().log(new Status(IStatus.ERROR, Activator.PLUGIN_ID, e.getMessage(), e));
					MessageDialog.openError(getShell(), "Deploy Workflow Error", e.getMessage());
	...
	public void selectionChanged(IAction action, ISelection selection) {
		selectedFile = (IFile) ((StructuredSelection) selection).getFirstElement();
	}
